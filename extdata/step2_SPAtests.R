...
option_list <- list(
 ...
 make_option("--minGroupMAC_in_BurdenTest", type="numeric", default=5,
  ...
  make_option("--is_output_cross_anc_cov", type="logical",default=FALSE,
    help="For admixed analysis (--is_admixed=TRUE): also output the per-variant cross-ancestry score covariance columns covT_anc{a}_anc{b}. These enable cross-ancestry genetic correlation (rho_w) estimation from the summary stats. By default, FALSE."),
  ...
  make_option("--rg_nJackknifeBlocks", type="integer",default=1,
    help="RHE: split this run's markers into this many block-jackknife blocks, writing one <SAIGEOutputFile>.block{b}.rg_partial.bin per block. Lets step3 produce an rg/h2 standard error from a SINGLE step2 job (each block is a leave-one-out unit). 1 = original single-file behavior; e.g. 20 for an SE from one run. Composes with multi-chunk runs (all blocks pooled). [default=1]"),
  make_option("--rg_pairs", type="character",default="",
    help="RHE: comma-separated ancestry pairs (1-indexed), e.g. '1-2,1-3,3-4', to estimate cross-ancestry rg COHERENTLY per pair on markers polymorphic in BOTH ancestries (M_ab; the radmix criterion). Empty (default) = joint all-K analysis on markers shared by all ancestries [default '']."),
  make_option("--rg_perAncestryH2", type="logical",default=FALSE,
    help="RHE: also estimate each ancestry's SNP heritability on its OWN markers (M_a, single-GRM h2) -- the proper per-ancestry heritability, free of the all-K intersection. Reported in the h2 table [default FALSE]."),
  make_option("--rg_markerFile", type="character",default="",
    help="RHE: restrict the cross-ancestry rg components to the markers listed in this file (one variant id or chr:pos per line). rg is a scale-free ratio and tolerates aggressive LD-pruning (e.g. ~200k), cutting cost. Empty = use every tested marker [default '']."),
  make_option("--h2_markerFile", type="character",default="",
    help="RHE: restrict the per-ancestry h2 components to the markers in this file. h2 is tagging-limited, so use a fuller set (e.g. HapMap3 ~1M), NOT an aggressively pruned one. Empty = use every tested marker [default '']."),
...
if(nThreads == 1){
  SPAGMMATtest(vcfFile=opt$vcfFile,
             ...
             is_output_cross_anc_cov = opt$is_output_cross_anc_cov,
             estimate_cross_anc_rg = opt$estimate_cross_anc_rg,
             rg_nProbes = opt$rg_nProbes,
             rg_seed = opt$rg_seed,
             rg_nJackknifeBlocks = opt$rg_nJackknifeBlocks,
             rg_pairs = opt$rg_pairs,
             rg_perAncestryH2 = opt$rg_perAncestryH2,
             rg_markerFile = opt$rg_markerFile,
             h2_markerFile = opt$h2_markerFile,

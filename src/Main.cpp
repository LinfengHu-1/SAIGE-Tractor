...
//////// ---------- Standalone block reader for the cross-ancestry RHE (rg/h2) --------- ////////////
// Reads a chunk of markers via the already-set-up tractor_hybrid reader and returns the
// EXPOSURE-CENTERED, AF-STANDARDIZED ancestry components
//   Z_a = (X_a - n_a * p_a) / sqrt(2 p_a (1-p_a))
// (X_a = "DS{a}" ancestry-a dosage, n_a = "ANC{a}" local-ancestry-a haplotype count,
// p_a = ancestry-a ALT freq) -- the SAME Z_a step2's RHE builds, but WITHOUT the
// association test / SPA / conditional scaffolding. JOINT-KEEP: a marker is returned only
// if EVERY ancestry passes the per-ancestry MAF floor. The RHE itself (probes, traces,
// jackknife, solve) is done in R (see step2_rgRHEonly.R) -- a direct port of the validated
// standalone_rg_h2.py. Returns a list of K matrices (N x nKept), each = Z_a for the kept
// markers of this chunk. Reuses ptr_gTRACTORHYBRIDobj (set up by the same R setup as step2).
// [[Rcpp::export]]
Rcpp::List readZaChunkInCPP(std::string t_genoType,
                            std::vector<std::string>& t_genoIndex,
                            int t_NumberofANC,
                            double t_mafFloor)
{
  // PURE block reader: returns per-ancestry Z_a + per-ancestry MAF-pass mask + marker id
  // (chr:pos) for EVERY successfully-read marker in the chunk (NO joint-keep -- the R
  // wrapper does ancestry-subset / per-pair M_ab / per-ancestry M_a / marker-file logic).
  // A marker is included iff its record exists (in range); out-of-range gIndex are skipped,
  // so an all-empty chunk (nRead=0) signals end-of-markers for the R auto-terminate loop.
  int q = (int) t_genoIndex.size();
  arma::uword n = (arma::uword) ptr_gTRACTORHYBRIDobj->getN();
  bool t_isImputation = false;

  std::vector< std::vector<arma::vec> > Zcols(t_NumberofANC);   // Zcols[a] -> N-vecs
  std::vector< std::vector<int> >       okCols(t_NumberofANC);  // okCols[a] -> MAF-pass 0/1
  std::vector<std::string> markerIDs;                          // variant id (or chr:pos fallback)
  std::vector<std::string> markerCPs;                          // chr:pos

  for(int i = 0; i < q; i++){
    uint64_t gIndexBase = std::strtoull(t_genoIndex.at(i).c_str(), NULL, 10);
    uint64_t gIndexPrevBase = (i > 0) ? std::strtoull(t_genoIndex.at(i-1).c_str(), NULL, 10) : 0;

    std::vector<arma::vec> Za_marker(t_NumberofANC);
    std::vector<int> ok_marker(t_NumberofANC, 0);
    bool recordRead = false; std::string mchr = ""; uint32_t mpd = 0; std::string mmk = "";
    for(int j = 0; j < t_NumberofANC; j++){
      // --- DS{j+1}: raw ancestry-a dosage X_a ---
      arma::vec Xa;
      std::vector<uint> idxMiss, idxNZ;
      std::string ref="", alt="", marker="", chr="";
      uint32_t pd=0;
      double altFreq=0, altCounts=0, missingRate=0, imputeInfo=0;
      bool outMiss=false, onlyNZ=false;
      uint64_t gP=gIndexPrevBase, gI=gIndexBase;
      std::string dsField = "DS" + std::to_string(j+1);
      Unified_getOneMarker_Admixed(t_genoType, gP, gI, ref, alt, marker, pd, chr,
        altFreq, altCounts, missingRate, imputeInfo, outMiss, idxMiss, onlyNZ, idxNZ,
        Xa, t_isImputation, dsField);
      if(Xa.n_elem != n) break;                 // out-of-range record -> skip this marker
      recordRead = true; mchr = chr; mpd = pd; mmk = marker;
      double altC = arma::accu(Xa);

      // --- ANC{j+1}: per-individual ancestry-a haplotype count n_a ---
      arma::vec na; na.zeros(Xa.n_elem);
      std::vector<uint> idxMiss2, idxNZ2;
      std::string ref2="", alt2="", marker2="", chr2="";
      uint32_t pd2=0;
      double aF2=0, aC2=0, mR2=0, iI2=0;
      bool outMiss2=false, onlyNZ2=false;
      uint64_t gP2=gIndexPrevBase, gI2=gIndexBase;
      std::string ancField = "ANC" + std::to_string(j+1);
      Unified_getOneMarker_Admixed(t_genoType, gP2, gI2, ref2, alt2, marker2, pd2, chr2,
        aF2, aC2, mR2, iI2, outMiss2, idxMiss2, onlyNZ2, idxNZ2,
        na, t_isImputation, ancField);
      double nanc = arma::accu(na);
      double pa = (nanc > 0) ? altC / nanc : 0.0;

      if(pa > t_mafFloor && pa < 1.0 - t_mafFloor){
        double afsd = std::sqrt(2.0 * pa * (1.0 - pa));
        Za_marker[j] = (Xa - pa * na) / afsd;   // exposure-centered AF-std Z_a
        ok_marker[j] = 1;
      }else{
        Za_marker[j] = arma::zeros<arma::vec>(n);
        ok_marker[j] = 0;
      }
    }
    if(recordRead){
      for(int j = 0; j < t_NumberofANC; j++){
        Zcols[j].push_back(Za_marker[j].n_elem == n ? Za_marker[j] : arma::zeros<arma::vec>(n));
        okCols[j].push_back(ok_marker[j]);
      }
      markerIDs.push_back(mmk.empty() ? (mchr + ":" + std::to_string(mpd)) : mmk);
      markerCPs.push_back(mchr + ":" + std::to_string(mpd));
    }
  }

  int m = (int) markerIDs.size();
  Rcpp::List ZaList(t_NumberofANC);
  Rcpp::IntegerMatrix okMat(t_NumberofANC, m);
  for(int j = 0; j < t_NumberofANC; j++){
    arma::mat Mz(n, m);
    for(int c = 0; c < m; c++){ Mz.col(c) = Zcols[j][c]; okMat(j, c) = okCols[j][c]; }
    ZaList[j] = Mz;
  }
  return Rcpp::List::create(Rcpp::Named("Za") = ZaList,
                            Rcpp::Named("ok") = okMat,
                            Rcpp::Named("markerID") = Rcpp::wrap(markerIDs),
                            Rcpp::Named("markerCP") = Rcpp::wrap(markerCPs),
                            Rcpp::Named("nRead") = m,
                            Rcpp::Named("nTotal") = q);
}
